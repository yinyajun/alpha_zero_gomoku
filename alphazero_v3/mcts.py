...
class TreeNode:
    ...
    def __init__(self, game: Game, model, parent: Optional["TreeNode"] = None):
        self.game = game
        self.model = model
        self.parent = parent
        ...
        if not self.is_terminal():
            self.untried_moves = self.game.feasible_moves()

    def is_terminal(self):
        return self.game.result != 0
    ...
    def ensure_priors_and_value(self):
        ...
        if self.is_terminal():
            ...
            self.value = -1.0 if self.game.result == Win else 0.0  # 上一手胜 => 我方输 => -1；平 => 0
            return
        ...
        mask = torch.from_numpy(self.game.board.flatten() != 0).bool()
    ...
    def expand(self, use_prior: bool = False) -> "TreeNode":
        """
        只在expand的过程中落子
        """
        idx = np.random.randint(len(self.untried_moves))  # 随机扩展
        ...
        child = TreeNode(game, self.model, self)
