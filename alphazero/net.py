...
class Alpha0Module(torch.nn.Module):
    ...
    def train_model(
        ...
        for epoch in range(1, epochs + 1):
            states, target_policies, target_values = buffer.sample_batch(batch_size, self.device)
            ...
            if epoch % log_interval == 0:
                print(f"Epoch: {epoch: > 4}, "
                      ...
                      f"PolicyEnt: {entropy.item(): .2f}, "
                      f"GradNorm: {grad_norm.item(): .2f}, "
                      f"TargetEnt: {target_entropy.item(): .2f}")
    ...
    def load_checkpoint(self, path: str):
        ...
        # 把优化器里的 tensor 状态迁移到当前 device（很重要）
